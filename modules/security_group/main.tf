...
resource "aws_security_group_rule" "s3_files_mounttarget_ingress_nfs_from_private_ec2_sg" {
  type                     = "ingress"
  from_port                = 2049
  to_port                  = 2049
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.this["private_ec2"].id
  security_group_id        = aws_security_group.this["s3_files_mounttarget"].id
